...
public class MaterialNames {
    ...
    public static String getItemName(String name) {
        if (name.contains(":")) {
            ...
            Short dataValue;
            try {
                id = Integer.valueOf(splitStr[0]);
                dataValue = Short.valueOf(splitStr[1]);
            } catch (NumberFormatException ex) {
                return null;
            }
            if ((id == null) || (dataValue == null)) {
                return null;
            }
            return getItemName(id, dataValue);
        }
        else {
            Integer id;
            try {
